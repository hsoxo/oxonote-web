...
const SortSetting: React.FunctionComponent = () => {
  ...
  const handleChange = (attrId: string, payload: any) => {
    ...
    let attrIndex = newViews[viewIndex].sorts.findIndex(
      x => x.attrId === attrId
    )
  ...
  if (!viewId) return <Button>排序</Button>

  const sorts = curViewInfo.sorts || []
  const availableAttrs = jourAttrs.filter(
    x => !sorts.some(y => y.attrId === x.attrId)
  )

  return (
    <div>
      <Button {...bindTrigger(popupState)}>排序</Button>
      <Popover
          ...
          {sorts.map((x, index) => (
            <DenseListItemBoxNoHover key={index}>
              <DenseSelect
                ...
                value={x.attrId}
                onChange={e =>
                  handleChange(x.attrId, { attrId: e.target.value })
                }
                displayEmpty
              >
              ...
              <DenseSelect
                ...
                value={x.direction}
                onChange={e =>
                  handleChange(x.attrId, { direction: e.target.value })
                }
                displayEmpty
              >
