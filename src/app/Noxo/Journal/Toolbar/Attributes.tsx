...
const AttributeSetting: React.FunctionComponent = () => {
  ...
  let attributes = jourAttrs
    ...
    .sort(
      (a, b) => journal.attrIds.indexOf(a._id) - journal.attrIds.indexOf(b._id)
    )

  const attrSetting = curView.attribute || []

  const handleToggle = (attrId: string) => {
    let newAttrSetting: Array<JournalViewAttribute> = JSON.parse(
      JSON.stringify(curView.attribute)
    )
    ...
    sagaAction({
      type: JOURNAL_ACT.SAGA_UPDATE_VIEW_ATTR_SETTING,
      viewId,
  ...
  const handleDragEnd = (result: DropResult) => {
    ...
    const newAttrSetting: Array<string> = JSON.parse(
      JSON.stringify(journal.attrIds)
    )
    ...
    sagaAction({
      ...
      payload: { attrs: newAttrSetting }
    })
  }

  return (
    <div>
      <Button {...bindTrigger(popupState)}>显示 / 隐藏属性</Button>
      <Popover
                      ...
                      {(provided, snapshot) => (
                        ...
                        >
                          <DenseListItemBox key={x.attrId}>
                            ...
                            <DenseListItemIcon>{x.icon}</DenseListItemIcon>
                            {x.label}
                            <ListItemSecondaryAction>
                              <AntSwitch
                                checked={
                                  (
                                    attrSetting.find(y => y.attrId === x._id) ||
                                    {}
                                  ).status || false
                                }
