...
const JournalKanbanView = () => {
  ...
  const { views, attrs: jourAttrs, notes }: JournalState = useSelector(state => state.get('journal'))
  ...
  const columnData = curAttr.range?.map(x => ({
    ...x,
    notes: notes.filter(n => n.attributes.find(y => y.attrId === curAttr._id)?.value === x.id)
  })) || []
  ...
  const handleDragEnd = (result: DropResult) => {
    ...
    if (type === "COLUMN") {
      ...
      if (Array.isArray(curAttr.range)) {
        const reordered = reorder(curAttr.range, source.index, destination.index)
        sagaAction({ type: SAGA_UPDATE_ATTR_RANGE, attrId: curAttr._id, newRange: reordered })
      }
    } else if (type === "NOTE") {
      if (curAttr.type === SINGLE_SELECT) {
        sagaAction({ type: SAGA_UPDATE_ATTRIBUTE_VALUE, noteId: draggableId, attrId: curAttr._id, newValue: destination.droppableId})
      }
    }
  }
      ...
      >
        {(provided, snapshot) => (
          ...
          >
            ...
            <Column id="0" label="未分类" color="#dddddd" notes={uncategorized} index={-1}/>}
