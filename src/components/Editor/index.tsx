...
const OxOEditor = ({ value, onChange }: OxOEditorProps) => {
  const editor = useMemo(
    () => withHistory(withPlugins(withReact(createEditor()), plugins)),
    []
  )
  ...
  editor.insertBreak = () => {
    ...
    if (selection && Range.isCollapsed(selection) && curBlock) {
      ...
      if (afterwardTextInBlock) {
        if (
          parentBlock.type === NAMES.BLOCKQUOTE ||
          parentBlock.type === NAMES.CODE
        ) {
          insertText('\n')
        } else {
        ...
        if (
          anchor.path.slice(-2)[0] === siblings.length - 1 &&
          forwardTextInBlock + afterwardTextInBlock === ''
        ) {
          ...
          Transforms.insertNodes(editor, {
            children: [{ text: '' }],
            type:
              curBlockInfo.type === NAMES.LIST_ITEM
                ? NAMES.LIST_ITEM
                : NAMES.PARAGRAPH
          })
        }
