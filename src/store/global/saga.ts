  ...
  while (true) {
    ...
    switch (action.type) {
      case ACT.SAGA_LOAD_JOURNAL_LIST: {
        yield fork(loadJournalList)
        break
      }
      case ACT.SAGA_LOGIN: {
        yield fork(sagaLogin, action.username, action.password)
        break
      }
      ...
      case ACT.SAGA_SIGN_UP: {
        yield fork(sagaSignUp, action.username, action.password, action.email)
        break
      }
      case ACT.SAGA_LOAD_USER: {
        yield fork(loadUserInfo, action.payload)
        break
      }
    }
  }
