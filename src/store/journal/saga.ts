...
const SAGA_ACTIONS = (Object.entries(ACT)
  .filter(x => x[0].startsWith('SAGA') && typeof x[1] === 'string')
  .map(x => x[1]) as unknown) as Array<string>
  ...
  while (true) {
    ...
    switch (action.type) {
      ...
      case ACT.SAGA_JOURNAL_READ: {
        yield fork(readJournal, action.journalId)
        break
      }
      case ACT.SAGA_UPDATE_INFO: {
        yield fork(updateJournalInfo, action.payload)
        break
      }
      case ACT.SAGA_UPDATE_ATTR_RANGE: {
        yield fork(journalAttrRangeChange, action.attrId, action.newRange)
        break
      }
      case ACT.SAGA_UPDATE_VIEW_ATTR_SETTING: {
        yield fork(updateViewAttrSetting, action.viewId, action.attribute)
        break
      }
      case ACT.SAGA_CREATE_VIEW: {
        yield fork(createJournalView, action.viewType, action.viewLabel)
        break
      }
      ...
      case ACT.SAGA_DELETE_VIEW: {
        yield fork(deleteJournalView, action.viewId)
        break
      }
    }
  }
...
function* saveToPouch() {
  try {
    const { journalChanged, journal }: JournalState = yield select(state =>
      state.get('journal')
    )
    if (journalChanged) {
...
function* journalAttrRangeChange(
  attrId: string,
  newRange: Array<AttributeRangeType>
) {
  try {
...
function* updateViewAttrSetting(
  viewId: string,
  attribute: Array<JournalViewAttribute>
) {
  try {
...
export type JournalSagaActions =
  | JournalCreate
  | JournalRead
  | UpdateJournalInfo
  | JournalAttrRangeChange
