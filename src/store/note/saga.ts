...
const SAGA_ACTIONS = Object.entries(ACT)
  .filter(x => x[0].startsWith('SAGA'))
  .map(x => x[1])
  ...
  while (true) {
    ...
    switch (action.type) {
      case ACT.SAGA_CREATE_NOTE: {
        yield fork(createNote, action.journalId)
        break
      }
      case ACT.SAGA_READ_NOTE: {
        yield fork(readNote, action.noteId)
        break
      }
      case ACT.SAGA_UPDATE_INFO: {
        yield fork(updateNoteInfo, action.payload)
        break
      }
      case ACT.SAGA_UPDATE_CONTENT: {
        yield fork(updateNoteContent, action.content)
        break
      }
      ...
      case ACT.SAGA_UPDATE_ATTRIBUTE_TYPE: {
        yield fork(updateNoteAttributeType, action.attrId, action.newType)
        break
      }
      case ACT.SAGA_UPDATE_ATTRIBUTE_TITLE: {
        yield fork(updateNoteAttributeTitle, action.attrId, action.newTitle)
        break
      }
      case ACT.SAGA_UPDATE_ATTRIBUTE_VALUE: {
        yield fork(
          updateNoteAttributeValue,
          action.noteId,
          action.attrId,
          action.newValue
        )
      ...
      case ACT.SAGA_UPDATE_ATTRIBUTE_SELECT_RANGE: {
        yield fork(
          updateNoteAttributeSelectRange,
          action.attrId,
          action.newRange
        )
        break
      ...
      case ACT.SAGA_REMOVE_ATTRIBUTE: {
        yield fork(removeNoteAttribute, action.attrId)
        break
      }
    }
  }
...
function* saveToPouch() {
  try {
    const { note }: NoteState = yield select(state => state.get('note'))
...
type UpdateNoteInfo = {
  type: typeof ACT.SAGA_UPDATE_INFO
  payload: PartialNoteObject
}
function* updateNoteInfo(payload: PartialNoteObject) {
  try {
    const {
      note: { _id }
    } = yield select(state => state.get('note'))
...
type UpdateNoteContent = {
  type: typeof ACT.SAGA_UPDATE_CONTENT
  content: NoteContent
}
...
type CreateNoteAttribute = {
  type: typeof ACT.SAGA_CREATE_ATTRIBUTE
  noteId: string
  attrType: string
  label: string
}
...
type UpdateNoteAttributeType = {
  type: typeof ACT.SAGA_UPDATE_ATTRIBUTE_TYPE
  attrId: string
  newType: string
}
...
type UpdateNoteAttributeTitle = {
  type: typeof ACT.SAGA_UPDATE_ATTRIBUTE_TITLE
  attrId: string
  newTitle: string
}
...
type UpdateNoteAttributeValue = {
  type: typeof ACT.SAGA_UPDATE_ATTRIBUTE_VALUE
  noteId: string
  attrId: string
  newValue: any
}
function* updateNoteAttributeValue(
  noteId: string,
  attrId: string,
  newValue: any
) {
...
type UpdateNoteAttributeSelectRange = {
  type: typeof ACT.SAGA_UPDATE_ATTRIBUTE_SELECT_RANGE
  attrId: string
  newRange: any
}
...
type RemoveNoteAttribute = {
  type: typeof ACT.SAGA_REMOVE_ATTRIBUTE
  attrId: string
}
...
export type NoteSagaAction =
  | CreateNote
  | ReadNote
  | UpdateNoteInfo
  | UpdateNoteContent
  | CreateNoteAttribute
  | UpdateNoteAttributeType
  | UpdateNoteAttributeTitle
  | UpdateNoteAttributeValue
  | UpdateNoteAttributeSelectRange
  | RemoveNoteAttribute
...
function* refreshNote() {
  try {
    ...
    const {
      journal: { _id: jid }
    } = yield select(state => state.get('journal'))
    if (jid) {
